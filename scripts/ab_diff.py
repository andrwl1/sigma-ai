...
def main() -> int:
    ...
    diff_mask = (merged["ver_local"].fillna("") != merged["ver_cloud"].fillna("")) | (
    ...
    # аккуратный вид в MD
    def clip(x):
        return shorten(str(x), width=120, placeholder="…")
    ...
    if diffs.empty:
        ...
        summary = {
            "verdict_only": (
                (merged["ver_local"] != merged["ver_cloud"])
                & (merged["got_local"] == merged["got_cloud"])
            ).sum(),
            "answer_only": (
                (merged["ver_local"] == merged["ver_cloud"])
                & (merged["got_local"] != merged["got_cloud"])
