...
SOFT_MODE="${SOFT_MODE:-false}"   # true => не валим билд, только warning
SRC_BRANCH="${GUARD_SOURCE_BRANCH:-master}"
SRC_WORKFLOW="${GUARD_WORKFLOW:-Pre-CI}"

echo ">>> Guard running for event=$CI_EVENT (branch=$SRC_BRANCH, workflow=$SRC_WORKFLOW)"
echo ">>> Threshold: ${THRESHOLD_PP_2}pp"
...
DELTA=$(python3 - <<PY
p=float("$PREV_RATE"); c=float("$CURR_RATE")
print(round(c-p,2))
PY
)
